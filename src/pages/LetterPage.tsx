...
const LetterPage = ({ onNavigate }: LetterPageProps) => {
  return (
    <section className="min-h-screen pt-20 pb-32 px-4 fade-in flex items-center justify-center">
      <div className="max-w-2xl w-full">
        <h2 className="text-5xl font-accent text-foreground mb-2 pl-2">Letter</h2>
        <p className="text-muted-foreground mb-8 pl-2 text-sm">To my next team and Employer</p>
        <div className="glass rounded-[2rem] p-8 md:p-12 shadow-2xl relative">
          <Button
            variant="ghost"
            onClick={() => onNavigate('home')}
            className="absolute top-6 right-8 text-muted-foreground hover:text-foreground text-sm"
          >
            ← Back Home
          </Button>
        ...
        <div className="mt-12 text-center text-muted-foreground text-xs">
          © {new Date().getFullYear()} Elewachi Emmanuel. All rights reserved.
        </div>
      </div>
    </section>
  );
